/*
 * main_menu.h is used for handleMenu() and 
 * has all the menu elements and prompts to other
 * states of the project app such as 
 * playing the game or displaying About
 */

#include "utils.h"
#include "game.h"

//Blinking pointer variables
unsigned long blinkTimer;
const int blinkDelay = 300;
const int resetBlinkDelay = 300 + 150;
...
// How-to-play & About menu variables
...
void displayChangeName() {
    ...
    if (currentPositionChangeName < 7) {
        ...
        case up:
            ...
            break;
        case down:
            if (newName[currentPositionChangeName] == 97) //lower case 'a'
            {
                newName[currentPositionChangeName] = 32; // upper case 'Z'
            } else if (newName[currentPositionChangeName] == 32) // empty character
            {
                newName[currentPositionChangeName] = 90; // upper case 'Z'
            } else if (newName[currentPositionChangeName] == 65) // upper case 'A'
            {
...
void displaySystemConfig() {
    ...
    if (currentInputRead != lastInputRead) {
        ...
        case enter:
            screenStatus = STATUS_SETTINGS;
            pointerMode = POINTER_SCROLL;
            writeIntIntoEEPROM(eepromDifficultyAddress, difficultyValue);
            writeIntIntoEEPROM(eepromReverseJoystickAddress, reverseJoystick);
...
void handleMenu() {
    ...
    if (screenStatus == STATUS_SETTINGS) {
        ...
        debounceDelay = 90;   // new debounceDelay for multiple almost-instant input: joystick & button
        int gameOver = runGame();
        if (gameOver == 1) {
            debounceDelay = 800; // old debounceDelay
            screenStatus = STATUS_MAINMENU;
            pointerMode = POINTER_SCROLL;
            currentMainMenuIndex = 0;
        }
